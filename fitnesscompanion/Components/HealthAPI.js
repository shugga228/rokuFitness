...
export const Calories = async () => {
    ...
    const startTime = new Date(currentTime.getFullYear(), currentTime.getMonth(), currentTime.getDate(), 0, 0, 0, 0);

    // For Android, use the API
    if (Platform.OS === "android") {
        // Wait for Android API to initialize if it hasn't
        const isInitialized = await initialize();
        if (isInitialized) {
            // Reads the current calories burned since midnight
